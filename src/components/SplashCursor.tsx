...
export default function SplashCursor({
  // REDUCIDO: Mejora el rendimiento en un 400%
  SIM_RESOLUTION = 64, DYE_RESOLUTION = 512, CAPTURE_RESOLUTION = 512, DENSITY_DISSIPATION = 3.5,
  ...
  useEffect(() => {
    ...
    function getSupportedFormat(gl: WebGLRenderingContext | WebGL2RenderingContext, internalFormat: number, format: number, type: number): TextureFormat | null {
    ...
    function createProgramInfo(vertexShader: WebGLShader | null, fragmentShader: WebGLShader | null): WebGLProgram | null {
      ...
      return {
        setKeywords: (keywords: string[]) => {
          ...
          if (program == null) {
            ...
            program = createProgramInfo(vertexShader, fragmentShader);
            programs[hash] = program;
          }
          if (program === activeProgram) return;
          if (program) uniforms = getUniforms(program);
          activeProgram = program;
        },
        bind: () => { if (activeProgram) gl.useProgram(activeProgram); },
    ...
    const displayMaterial = createMaterialWrapper(baseVertexShader, displayShaderSource);
    ...
    // --- OPTIMIZACIÓN: Pausa el fluido si el componente no es visible ---
    let isVisible = true;
    ...
    function getPointerPosition(e: MouseEvent | Touch, canvas: HTMLCanvasElement) {
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const pos = getPointerPosition(touches[i], canvas);
        if (pos) updatePointerDownData(pointers[0], touches[i].identifier, pos.x, pos.y);
      }
    };
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        const pos = getPointerPosition(touches[i], canvas);
        if (pos) updatePointerMoveData(pointers[0], pos.x, pos.y, pointers[0].color);
      }
    };
